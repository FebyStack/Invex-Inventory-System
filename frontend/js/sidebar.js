 ...
 * Loads sidebar from /components/sidebar.html, then:
...
(function () {
  ...
  // ── Load sidebar component if placeholder exists, then init ──
  const placeholder = document.getElementById('sidebar-placeholder');

  if (placeholder) {
    // Fetch the shared sidebar HTML and inject it
    fetch('/components/sidebar.html')
      .then(res => res.ok ? res.text() : Promise.reject('Failed to load sidebar'))
      .then(html => {
      ...
      .catch(err => {
        ...
        initSidebar(); // still try to init in case inline sidebar exists
      });
  } else {
    // Sidebar is already inline in the page (e.g. dashboard.html)
    initSidebar();
  }
