-- Location-specific SKU identities.
-- A product keeps its original global SKU, while each product/location stock row
-- can hold the SKU that belongs to the location where those units reside.
...
CREATE OR REPLACE VIEW invex.transfer_log_details AS
SELECT
    t.id,
    t.quantity,
    t.notes,
