...
async function runTest() {
  try {
    ...
    const orderRes = await query(
      `INSERT INTO orders (order_type, source_location_id, destination_location_id, user_id, status)
    ...
    await query(
      `INSERT INTO stock_adjustments (product_id, location_id, reason_code_id, adjustment_type, quantity, user_id, remarks)
    ...
    const res = await query(
      `SELECT * FROM stock_movements WHERE product_id = 1 ORDER BY movement_date DESC LIMIT 5;`
    );
